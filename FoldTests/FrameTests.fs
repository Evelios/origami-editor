...
let testCases: (string * Frame<TestCoordinates>) list =
    [ """{"frame_unit":"unit"}""",
      { Frame.empty with
      ...
      { Frame.empty with
          Classes = Set.ofList [ FrameClass.CreasePattern; FrameClass.Graph ] }

      """{"frame_attributes":["2D","orientable"],"frame_unit":"unit"}""",
      { Frame.empty with
          Attributes = Set.ofList [ FrameAttribute.Geo2D; FrameAttribute.Orientable ] }
      """{"frame_unit":"unit","vertices_coords":[[-1,-1],[1,1]]}""",
      { Frame.empty with
          Vertices =
              { Vertices.empty with
                  Coordinates = [ Point2D.meters -1. -1.; Point2D.meters 1. 1. ] } }
      """{"frame_unit":"unit","vertices_coords":[[0.5,1],[2.5,2.5]]}""",
      { Frame.empty with
          Vertices =
              { Vertices.empty with
                  Coordinates = [ Point2D.meters 0.5 1.; Point2D.meters 2.5 2.5 ] } }
      ...
      { Frame.empty with
          Edges =
              { Edges.empty with
                  Assignment = [ EdgeAssignment.Boundary; EdgeAssignment.Unassigned ] } }
