...
let redundantElementsTestCases =
    let v1 = Point2D.meters 0. 0.
    let v2 = Point2D.meters 1. 1.
...
let ``Get edges from crease pattern`` () =
    let edges =
        [ Point2D.meters 0. 0., Point2D.meters 1. 1., EdgeAssignment.Flat
          Point2D.meters 1. 0., Point2D.meters 0. 1., EdgeAssignment.Flat
          Point2D.meters 1. 1., Point2D.meters 1. 1., EdgeAssignment.Flat ]
...
let pointDistanceTestCases =
    ...
    [ ([], Point2D.meters 0.1 0.1, Point2D.meters 0. 0., sqrt (0.1 ** 2. + 0.1 ** 2.))
      ([], Point2D.meters 0.7 0.9, Point2D.meters 1. 1., sqrt (0.1 ** 2. + 0.3 ** 2.))
      ([ Point2D.meters 0.5 0.5 ], Point2D.meters 0.6 0.65, Point2D.meters 0.5 0.5, sqrt (0.1 ** 2. + 0.15 ** 2.)) ]
    |> List.map
...
let pointCloseTestCases =
    ...
    [ ("Point Not Close", pointAdd, Point2D.meters 0.1 0.1, None)
      ("Point Close", pointAdd, Point2D.meters 0.5 0.45, Some pointAdd)
      ("Point On Boundary", pointAdd, Point2D.meters 0.5 0.4, Some pointAdd) ]
...
let edgeCloseTestCases =
    let edgeAdd =
        Edge.betweenWithAssignment (Point2D.meters 0. 0.5) (Point2D.meters 0.5 0.5) EdgeAssignment.Unassigned

    [ ("Point Not Close", edgeAdd, Point2D.meters 0.1 0.1, None)
      ("Point on line", edgeAdd, Point2D.meters 0.3 0.5, Some edgeAdd)
      ("Point near perpendicular", edgeAdd, Point2D.meters 0.3 0.5, Some edgeAdd)
      ("Point near endpoint", edgeAdd, Point2D.meters 0.55 0.5, Some edgeAdd) ]
    |> List.map
        (fun (name, add, test, result) ->
            TestCaseData(add, test)
                .Returns(result)
