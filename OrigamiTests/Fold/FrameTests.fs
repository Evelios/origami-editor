...
let testCases: (string * Frame<TestSpace>) list =
    [ """{"frame_unit":"unit"}""", { Frame.empty with Unit = LengthUnit.Unitless }
      """{"frame_author":"The Author","frame_unit":"unit"}""", { Frame.empty with Author = "The Author" }

      """{"frame_title":"The Title","frame_unit":"unit"}""", { Frame.empty with Title = "The Title" }

      """{"frame_description":"The Description","frame_unit":"unit"}""",
      { Frame.empty with Description = "The Description" }

      """{"frame_classes":["creasePattern","graph"],"frame_unit":"unit"}""",
      { Frame.empty with
          Classes =
              Set.ofList [
                  FrameClass.CreasePattern
                  FrameClass.Graph
              ] }
      ...
      { Frame.empty with Vertices = { Vertices.empty with Vertices = [ 0; 1; 2 ] } }
      """{"frame_unit":"unit","vertices_faces":[[0,1,2]]}""",
      { Frame.empty with Vertices = { Vertices.empty with Faces = [ [ 0; 1; 2 ] ] } }
      """{"frame_unit":"unit","edges_vertices":[[1,0],[0,2]]}""",
      { Frame.empty with Edges = { Edges.empty with Vertices = [ (1, 0); (0, 2) ] } }
      """{"frame_unit":"unit","edges_faces":[[1,2],[2,3]]}""",
      { Frame.empty with Edges = { Edges.empty with Faces = [ (1, Some 2); (2, Some 3) ] } }
      """{"frame_unit":"unit","edges_faces":[[1,null],[2,null]]}""",
      { Frame.empty with Edges = { Edges.empty with Faces = [ (1, None); (2, None) ] } }
      ...
      { Frame.empty with Faces = { Faces.empty with Edges = [ [ 1; 2; 3 ]; [ 3; 4; 1 ] ] } }
      """{"frame_unit":"unit","faceOrders":[[0,2,-1],[1,4,0]]}""",
      { Frame.empty with Faces = { Faces.empty with Orders = [ (0, 2, -1); (1, 4, 0) ] } } ]
...
let serializationTestCases =
    Util.toTestReverse testCases

[<TestCaseSource("serializationTestCases")>]
let Serialization source = Frame.toJson source
