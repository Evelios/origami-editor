...
type GraphElement =
    | EdgeElement of Edge
    | VertexElement of Point2D<Meters, OrigamiCoordinates>
...
[<RequireQualifiedAccess>]
type AxiomAction =
    | One of Point2D<Meters, OrigamiCoordinates> * Point2D<Meters, OrigamiCoordinates>
    | Two of Point2D<Meters, OrigamiCoordinates> * Point2D<Meters, OrigamiCoordinates>
    | Three of Line2D<Meters, OrigamiCoordinates> * Line2D<Meters, OrigamiCoordinates>
...
module Axiom =
    ...
    let betweenElements (e1: GraphElement) (e2: GraphElement) : AxiomAction list =
        match e1, e2 with
        | VertexElement p1, VertexElement p2 -> [ AxiomAction.One(p1, p2); AxiomAction.Two(p1, p2) ]
        | EdgeElement e1, EdgeElement e2 -> [ AxiomAction.Three(Edge.line e1, Edge.line e2) ]
        | _ -> []
    ...
    let first v1 v2 : Line2D<Meters, 'Coordinates> = Line2D.through v1 v2

    let second v1 v2 : Line2D<Meters, 'Coordinates> =
        Line2D.through v1 v2 |> Line2D.perpThroughPoint (Point2D.midpoint v1 v2)
    ...
    let third e1 e2 : Line2D<Meters, OrigamiCoordinates> list =
        // 1. Lines are equal
        if e1 = e2 then
            []
            ...
            | Some intersection ->
                let linePoint = if e1.Start = intersection then e1.Finish else e1.Start
                ...
                [ Line2D.perpThroughPoint intersection acuteLine; acuteLine ]

    /// Get the result of performing an axiom action
    let perform action : Set<Line2D<Meters, OrigamiCoordinates>> =
        match action with
